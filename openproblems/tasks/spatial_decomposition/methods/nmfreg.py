...
def nmfreg(adata):
    ...
    factor_to_cluster_map = np.array(
        [
            np.histogram(cluster_df.loc[cluster_df.factor == k,'code'],
                bins=n_types,range=(0,n_types))[0] for k in range(factors)
        ]
    ).T
    ...
    pos_neg_dict = {
        i : [sc_deconv[cluster_df.predicted_code == i,i],sc_deconv[cluster_df.predicted_code != i,i]] for i in range(n_types)
    }
    ...
    bead_prop_soln = np.array([scipy.optimize.nnls(Wa.T, X_sp_scaled[b,:])[0] for b in range(X_sp_scaled.shape[0])])
